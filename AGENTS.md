...
of any video from the keyboard. One source tree, built with Vite, React,
TypeScript and `@crxjs/vite-plugin`, builds both the Chrome Web Store and the
addons.mozilla.org package; only manifest keys differ.

- Key commands
  - `pnpm lint && pnpm typecheck && pnpm test`, the gate for every change.
  - `pnpm build`, `pnpm build:firefox`, writing `dist/` and `dist-firefox/`.
  ...
  - `pnpm inspect:chrome [url]`, a JSON snapshot of the running extension. Get
...
- What is actually frozen
  ...
  real constraints. Everything else is free to change: module boundaries,
  message shapes, the build pipeline, the test layout. The prior architecture
  does not get a vote.
