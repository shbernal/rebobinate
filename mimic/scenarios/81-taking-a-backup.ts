 ...
 *
 * And the status line under the whole thing appears on a press and is gone the
 * moment the other half is chosen, which is a lifetime.
 *
...
export default {
  ...
  async run(s: Ctx) {
    ...
    const exportHalf = panel.getByRole('button', { name: 'Export', exact: true })
    const importHalf = panel.getByRole('button', { name: 'Import', exact: true })
    ...
    const rule = panel.locator('.backup .rule')
    ...
    const importSlot = await paneContentHeight(panel)
    ...
    await look(
      s,
      panel,
      `"Import" has been pressed. The pair is one switch over one slot rather than two panels, so nothing opened underneath: the filled half moved from left to right and everything below it was replaced where it stood — the JSON is gone from the box, which now holds grey placeholder text reading "Paste a backup from Export on your other computer"; "Copy" has become "Replace settings", greyed out and unpressable; and the line under it is no longer about what the text is but about what pressing that button would cost: "${waiting.trim()}" The slot is ${importSlot === exportSlot ? 'exactly the same height it was' : `${Math.abs(importSlot - exportSlot)}px ${importSlot > exportSlot ? 'taller' : 'shorter'} than it was`}, which is the difference between two lines of explanation and one; nothing above the "Backup" heading moved. The "${copied}" line from a moment ago has gone with the switch, so the only trace that anything was copied is that it is on the clipboard.`,
    ...
    await look(
      s,
      panel,
      `The paste has been done again, all of it this time. Nothing else was touched and no button was pressed, and two things under the box have changed together: "Replace settings" has come alive — it is a solid, pressable button rather than a grey one — and the line beside it has gone back from the complaint to the price: "${costs.trim()}" That sentence is the whole of the confirmation. There is no "are you sure" after the press, deliberately, because this panel closes the instant anything outside it is clicked and a second step is one more way to lose the paste; the warning is put in front of the button instead of behind it.`,
    ...
    await look(
      s,
      panel,
      `"Replace settings" has been pressed. Everything it warned about happened — the settings and all ${SEEDED.length} remembered sites were overwritten — and the panel says so in one word, on the same line that reported the copy earlier: "${done}" The pair has flipped itself back to "Export" and the box is filled in again, this time from what is now stored; it is character for character the text that was copied out of it, which on one computer is the whole of what a restore can be shown to have done. What it does not say is which of the two things it replaced, or that anything was different before it ran.`,
      { name: 'restored', mustShow: status },
    )

    s.showVideo(
      `The same visit as a recording, at real speed, filmed in a window the width of the panel and the height it tops out at; the Settings tab is taller than the window, which is why it is scrolled. In order: the Settings tab is picked and the pane wheeled all the way down to the "Backup" section at its foot; "Copy" is pressed and "${copied}" appears under it; "Import" is pressed and the whole slot changes over in place — the JSON out of the box, the button from "Copy" to a dead "Replace settings", the line under it from what the text is to what replacing it costs, and the "${copied}" gone; a partial backup is pasted in and the button stays dead while the line under it turns into "${complained}"; the whole backup is pasted over it and the button comes alive as the line goes back to the price; and it is pressed, leaving "${done}" and the pair back on Export. The thing to watch is the second paste — the button and the sentence beside it change on the text itself, with nothing pressed in between.`,
    )
  },
}
